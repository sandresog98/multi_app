...
-- Tablas de Sifone
    ...
    CREATE VIEW sifone_resumen_asociados_vw AS
        ...
        FROM sifone_asociados AS sa
            ...
            LEFT JOIN (SELECT cedula, SUM(ABS(COALESCE(salant, 0))) AS aportes_sociales
                        FROM sifone_balance_prueba
                        WHERE nombre = 'APORTES SOCIALES 2'
                        GROUP BY cedula) sbp_as ON sbp_as.cedula = sa.cedula
            LEFT JOIN (SELECT cedula, SUM(ABS(COALESCE(salant, 0))) AS aportes_revalorizaciones
                        FROM sifone_balance_prueba
                        WHERE nombre = 'Revalorizacion Aportes'
                        GROUP BY cedula) sbp_ar ON sbp_ar.cedula = sa.cedula
            LEFT JOIN (SELECT cedula, SUM(ABS(COALESCE(salant, 0))) AS plan_futuro
                        FROM sifone_balance_prueba
                        WHERE nombre = 'PLAN FUTURO'
                        GROUP BY cedula) sbp_pf ON sbp_pf.cedula = sa.cedula
            LEFT JOIN (SELECT cedula, SUM(credit) AS bolsillos
                        FROM `sifone_movimientos_tributarios`
                        WHERE cuenta = '42309501'
                        AND fecham >= '2025-06-09'
                        AND credit > 0
                        GROUP BY cedula) AS smt_b ON smt_b.cedula = sa.cedula
    ;
