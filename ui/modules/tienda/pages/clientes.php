...
function renderCli(list){ const body=document.getElementById('tblCli'); body.innerHTML=''; if(!list.length){ body.innerHTML='<tr><td colspan="6" class="text-muted">Sin clientes</td></tr>'; return; } list.forEach(it=>{ const tr=document.createElement('tr'); const used = Number(it.usado||0) > 0; const btnDel = used ? '<span class="text-muted">Bloqueado</span>' : `<button class=\"btn btn-sm btn-outline-danger\" onclick=\"delCli(${Number(it.id)})\"><i class=\"fas fa-trash\"></i></button>`; tr.innerHTML=`<td>${escapeHtml(it.nombre||'')}</td><td>${escapeHtml(it.nit_cedula||'')}</td><td>${escapeHtml(it.telefono||'')}</td><td>${escapeHtml(it.email||'')}</td><td>${escapeHtml(it.fecha_creacion||'')}</td><td class=\"text-end\"><button class=\"btn btn-sm btn-outline-info\" onclick='editCli(${JSON.stringify(it)})'><i class=\"fas fa-edit\"></i></button> ${btnDel}</td>`; body.appendChild(tr); }); }
